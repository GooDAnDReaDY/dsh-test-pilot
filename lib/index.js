...
export const inject = ['tools', 'subprocess', 'settings', 'fs', 'connection', 'sessionController'];
...
export function apply(ctx, config = {}) {
  ...
  if (ctx.connection?.fetch?.register && ctx.sessionController?.list) {
    ...
    try {
      ...
      try { ctx.logger?.warn?.('dsh-test-pilot: session status endpoint is unavailable'); } catch {}
    }
  }
  if (typeof ctx.inject === 'function') {
    try {
