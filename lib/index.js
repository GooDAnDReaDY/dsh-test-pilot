...
import { createRunPersistence } from './persistence.js';
...
export function apply(ctx, config = {}) {
  ...
  const persistence = createRunPersistence({
    onError: (error) => {
      try { ctx.logger?.warn?.('dsh-test-pilot: persistent state is unavailable', error); } catch {}
    },
  });
  state.ready = persistence.ready.then((records) => state.restore(records));
  ...
  const finishRun = async (cwd, key, result) => {
    ...
    if (finished.status !== 'no-tests') {
      try { await persistence.recordResult(cwd, finished); } catch (error) {
        try { ctx.logger?.warn?.('dsh-test-pilot: could not save completed run state', error); } catch {}
      }
    }
    return finished;
  };
  ...
  const runNow = async (cwdOverride = '', event = null, session = null) => {
    ...
    return enqueueWorkspaceRun(cwd, async () => {
      ...
      return finishRun(cwd, id, normalizeRunResult({ ...result, testScope }));
    });
  };
  ...
  function clearRecordSignal(record) {
    if (record.signal && record.signalAbort) {
      try { record.signal.removeEventListener('abort', record.signalAbort); } catch {}
    }
    record.signal = null;
  ...
  function bindRecordSignal(record, signal) {
    ...
    record.signalAbort = () => {
      ...
      record.timer = null;
      if (record.controller) {
        try { record.controller.abort(signal.reason); } catch {}
        record.controller = null;
      }
    };
  ...
  async function startAutomaticRun(record) {
    ...
    const finish = async (result, testScope, publish = true) => {
      ...
      const finished = await finishRun(cwd, runId, {
      ...
      if (record.ended && record.reportedGeneration < generation) {
    ...
    try {
      ...
      const selection = await planChangedTests({
