...
export function apply(ctx, config = {}) {
  ...
  async function startAutomaticRun(record) {
    ...
    const finish = (result, testScope, publish = true) => {
      if (superseded()) {
        state.finish(runId, {
      ...
      const finished = state.finish(runId, {
    ...
    try {
      const testConfig = await resolveWorkspaceTestConfig(ctx.fs, cwd, cfg);
      if (superseded()) return finish({}, null, false);
      if (!testConfig?.enabled || !testConfig.runner) {
        finish({
      ...
      if (selection.kind === 'none') {
        finish({ status: 'no-tests', output: selection.reason }, selection, false);
        return;
      }
