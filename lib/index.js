...
export const Config = Schema.object({
  ...
  runner: Schema.string().default('pytest').description('MVP runner: pytest. Jest/Vitest parser is available for compatibility.'),
...
export function apply(ctx, config = {}) {
  ...
  const onTurnEnd = (session, event) => {
    ...
    void (async () => {
      ...
      if (cfg.skipIfNoChanges) {
        ...
        if (change.known && !change.changed) {
          const result = state.finish(key, {
            status: 'no-tests', runner: cfg.runner, command: cfg.command, cwd,
      ...
      const result = state.finish(key, {
        status: 'error', runner: cfg.runner, cwd: sessionCwd(session, event, cfg.cwd),
