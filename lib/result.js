...
function normalizeTestScope(scope) {
  if (!scope || !['none', 'related', 'full'].includes(scope.kind)) return null;
  return {
    kind: scope.kind,
    source: redactText(scope.source || '', 40),
    changedFiles: Array.isArray(scope.changedFiles) ? scope.changedFiles.slice(0, 32).map((path) => redactText(path, 240)) : [],
    paths: Array.isArray(scope.paths) ? scope.paths.slice(0, 32).map((path) => redactText(path, 240)) : [],
    reason: redactText(scope.reason || '', 400),
  };
}
export function normalizeRunResult(input = {}) {
  ...
  return {
    status, runner: redactText(input.runner || 'unknown', 80), command: redactText(Array.isArray(input.command) ? input.command.join(' ') : input.command || '', 240),
    ...
    testScope: normalizeTestScope(input.testScope),
...
export function formatRunReport(result) {
  ...
  if (value.testScope) {
    const target = value.testScope.paths.length ? ' · ' + value.testScope.paths.slice(0, 4).join(', ') : '';
    const reason = value.testScope.reason ? ' · ' + value.testScope.reason : '';
    lines.push('scope=' + value.testScope.kind + target + reason);
  }
