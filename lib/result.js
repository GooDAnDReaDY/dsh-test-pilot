export const RUN_STATUSES = Object.freeze(['passed', 'failed', 'error', 'timeout', 'no-tests']);
...
export function formatRunReport(result) {
  ...
  const icon = value.status === 'passed' ? '✅' : value.status === 'timeout' ? '⏱️' : '❌';
  ...
  if (!countParts.length) countParts.push(value.status === 'no-tests' ? 'no tests' : 'no test counts');
