...
function collectHandleOutput(handle, maxBytes) {
  ...
  if (handle?.collected?.stdout?.readFrom) {
    try { stdout.push(handle.collected.stdout.readFrom(0).text || ''); } catch (readError) { void readError; }
  }
  if (handle?.collected?.stderr?.readFrom) {
    try { stderr.push(handle.collected.stderr.readFrom(0).text || ''); } catch (readError) { void readError; }
  }
...
export async function runTestCommand({
  ...
  maxOutputBytes = 200000, signal, correlationId = '', args = [],
} = {}) {
  const selectedRunner = String(runner || 'pytest').toLowerCase();
  let argv;
  try {
    const extraArgs = Array.isArray(args) ? args : [];
    if (extraArgs.some((arg) => typeof arg !== 'string' || !arg || arg.includes(String.fromCharCode(0)))) {
      throw new Error('test arguments must be non-empty strings without NUL bytes');
    }
    argv = [...parseCommand(command || defaultCommand(selectedRunner)), ...extraArgs];
  } catch (error) {
  ...
  try {
    ...
    timer = setTimeout(() => {
      timedOut = true;
      abort();
      try { handle.terminate?.(); } catch (terminateError) { void terminateError; }
