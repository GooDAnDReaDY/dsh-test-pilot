  ...
  factory: (require) => {
    ...
    function SettingsForm(props) {
      ...
      const scope = React.useMemo(() => {
        try { return ctx && ctx.configForms && ctx.configForms.get ? ctx.configForms.get(NS) : null } catch { return null }
      }, [ctx])
    ...
    function registerLocale(ctx) {
      try { return ctx.locale.register(NS, TEXT) || (() => {}) }
        ...
        try { ctx.logger && ctx.logger.warn && ctx.logger.warn('dsh-test-pilot: locale registration failed', error) } catch (logError) { void logError; }
        return () => {}
      }
    }
    function apply(ctx) {
      ...
      try {
        ...
        try { ctx.logger && ctx.logger.warn && ctx.logger.warn('dsh-test-pilot: settings card registration failed', error) } catch (logError) { void logError; }
      }
      try {
        ...
        try { ctx.logger && ctx.logger.warn && ctx.logger.warn('dsh-test-pilot: session status chip registration failed') } catch (logError) { void logError; }
      }
    }
