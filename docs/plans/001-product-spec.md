...
После изменения файла плагин ждёт две секунды тишины, запускает тесты в фоне и
возвращает готовый короткий отчёт агенту через additionalContexts до конца
хода, когда есть следующий вызов инструмента: runner, статус,
passed/failed/skipped, длительность, область тестирования и причина полного
прогона. turn/end немедленно сбрасывает ожидающую проверку как страховку. Падение не должно теряться в контексте и не должно
...
Плагин состоит из host-половины и минимальной client-поверхности. Источниками являются tools/post-execute для успешных файловых мутаций и
session/event как страховка на turn/end. События tree/settled подключать
только после проверки их наличия в текущем host API.
...
- Hook tools/post-execute на успешные file-write инструменты; debounce
  2 секунды на сессию/ход, повторные записи сбрасывают таймер.
- Фоновый lifecycle queued -> running -> terminal; post-execute не ждёт процесс,
  наблюдает exec.signal и добавляет в additionalContexts только готовый
  результат, ещё не доставленный в этом ходе.
- turn/end немедленно запускает ожидающую проверку, если после последних
  изменений нет актуального результата.
- Отслеживание изменений текущего хода через нативный DSH workspaceChanges
  (fallback старого core — только успешные встроенные write/edit вызовы), без Git status.
- Выбор связанных тестов по соглашениям проекта; при неполном сопоставлении
  запускается полный набор с причиной; недоступная или неполная native-сводка
  тоже считается неполной. Без изменений автоматический subprocess
  не запускается; ручной test_pilot_run всегда запускает полный набор.
...
- 0.1.0 — один runner, tools/post-execute debounce с turn/end страховкой,
  фоновый lifecycle, parser, in-turn отчёт и bounded in-memory run state.
